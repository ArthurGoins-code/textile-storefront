    ...
    <header className="sticky top-0 z-50 border-b border-slate-800 bg-slate-950/95 backdrop-blur supports-[backdrop-filter]:bg-slate-950/80">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
          ...
          {navLinks.map((link) => (
            <Link
              ...
              className={clsx(
                ...
                pathname === link.href ? "text-amber-400" : "text-slate-200",
              )}
            >
              {link.label}
            </Link>
        ...
        <div className="flex items-center gap-2">
          <Link
            href="/cart"
            className="relative flex items-center gap-2 rounded-full border border-slate-700 px-3 py-2 text-sm font-medium text-slate-100 transition-colors hover:border-amber-400 hover:text-amber-400"
          >
